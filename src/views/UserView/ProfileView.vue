              ...
              <v-row>
                <v-col cols="12" md="6">
                  <v-text-field variant="solo-filled" v-model="user.firstName" label="ชื่อ"
                    prepend-inner-icon="mdi-account" :rules="[rules.required, rules.notEmpty]" required></v-text-field>
                </v-col>
                ...
                <v-col cols="12" md="6">
                  <v-text-field variant="solo-filled" v-model="user.email" label="อีเมล" prepend-inner-icon="mdi-email"
    ...
    async saveProfile() {
      ...
      if (await this.validateForm()) {
        try {
          ...
          errorStore.show("บันทึกข้อมูลสําเร็จ", {
            color: 'success',
