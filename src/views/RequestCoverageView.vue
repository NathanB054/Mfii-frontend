<template>
  ...
  <div v-if="detailsItems && detailsItems.length > 0" class="flex flex-col items-center space-y-4 p-4 bg-gray-100 min-h-screen">
  <div>
    ...
    <div class="flex flex-wrap justify-center gap-4 sm:gap-6 lg:flex-nowrap lg:justify-between lg:w-[1240px]">
      <div
        ...
        :class="{'bg-blue text-white': item.show, 'bg-white text-black': !item.show}"
      >
        <!-- Title Section -->
        ...
        <div class="icon-container">
          <i
            v-if="item.title === 'สิทธิบัตรการประดิษฐ์/อนุสิทธิบัตร'"
            class="mdi mdi-lightbulb-on-outline text-xl"
          ></i>
          <i
            v-if="item.title === 'สิทธิบัตรการออกแบบผลิตภัณฑ์'"
            class="mdi mdi-palette-outline text-xl"
          ></i>
          <i
            v-if="item.title === 'ลิขสิทธิ์'"
            class="mdi mdi-book-lock-outline text-xl"
          ></i>
          <i
            v-if="item.title === 'เครื่องหมายการค้า'"
            class="mdi mdi-tag-outline text-xl"
          ></i>
          <i
            v-if="item.title === 'ระบบติดตามผลงานที่อยู่ระหว่างดำเนินการยื่นคำขอฯ'"
            class="mdi mdi-eye-outline text-xl"
          ></i>
        </div>
      </div>
    </div>
...
.bg-blue-500 {
  background-color: #3b82f6; /* Adjust to the blue you want */
}
