    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
      ...
      <div class="container mx-auto max-w-6xl">
        ...
        <div class="mb-8 text-center">
          <h1 class="text-3xl font-bold text-gray-800 mb-2">
            จัดการสร้างความตระหนัก, การใช้ประโยชน์,
            และงานยกระดับงานวิจัยและนวัตกรรม
          </h1>
        </div>
        ...
        <div class="bg-white shadow-xl rounded-2xl p-6 mb-8">
          <div class="grid md:grid-cols-2 gap-6 items-center">
            <!-- Image Preview Area -->
            <div
              class="bg-gray-100 border-2 border-dashed border-gray-300 rounded-2xl p-6 flex flex-col items-center justify-center text-center min-h-[700px]"
            ...
            >
              ...
              <template v-if="!dataInfo.FilePreview">
                <v-icon class="text-6xl text-gray-400 mb-4"
              ...
              <template v-else>
                <div class="relative group w-full">
                  <!-- เช็คว่าเป็นรูปภาพ -->
                  <img
                    v-if="dataInfo.FilePreview[0].includes('image')"
                    :src="
                      dataInfo.id
                        ? `${baseUrl}/${dataInfo.FilePreview}`
                        : `${dataInfo.FilePreview}`
                    "
                  ...
                  <div v-else-if="dataInfo.FilePreview[0].includes('.pdf')">
                    <iframe
                      v-if="dataInfo.id"
                      :src="`${baseUrl}/${dataInfo.FilePreview}#toolbar=0`"
                      class="yes max-w-full min-h-[500px] rounded-xl mx-auto"
                      frameborder="0"
                      width="100%"
                      height="100%"
                    >
                  ...
                  <div
                    v-else-if="
                      dataInfo.FilePreview.includes('data:application/pdf')
                    "
                  >
                    <iframe
                      :src="`${dataInfo.FilePreview}#toolbar=0`"
                      class="yes max-w-full min-h-[500px] rounded-xl mx-auto"
                      frameborder="0"
                      width="100%"
                      height="100%"
                  ...
                  <div
                    :class="
                      dataInfo.FilePreview[0].includes('.pdf')
                        ? 'mt-2'
                        : 'absolute inset-0 group-hover:bg-opacity-20 flex items-center justify-center transition-all duration-300 rounded-xl '
                    "
                  >
                    <button
                      @click="triggerFileInput"
            ...
            <div>
              <v-form @submit.prevent="uploadImage">
                ...
                <v-autocomplete
                  ...
                  flat
                  label="หมวดหมู่"
                  v-model="servicesForApi.servicesType"
                  :items="dropdownItems"
                ...
                <v-text-field
                  v-model="dataInfo.servicesSubType"
                  label="ชื่อหัวข้อ"
                  variant="outlined"
                  class="mb-4"
                  :rules="[rules.topic]"
                ...
                <div class="flex space-x-4">
                  ...
                  <v-btn
                    ...
                    color="green darken-1"
                    variant="elevated"
                    @click="updateImagenData"
                    :disabled="
                      !dataInfo.FilePreview ||
                      !servicesForApi.servicesType ||
                      !dataInfo.information ||
                      !dataInfo.servicesType
                    "
                    class="flex-grow"
                  >
                    แก้ไขข้อมูล
                  ...
                  <v-btn
                    v-if="dataInfo.FilePreview && dataInfo.id"
      ...
      <v-dialog v-model="deleteDialog" max-width="400">
        <v-card class="rounded-xl py-2 px-2">
          <v-card-title class="headline text-red-800 text-2xl"
    ...
    async fetchData() {
      ...
      try {
        const res = await api.get(
          `/getsServices/${this.servicesForApi.servicesType}/all`
        );
        ...
        if (
          ...
          this.dataInfo = {
            ...
            servicesType:
              res.data.result[0]?.servicesType ||
    ...
    async updateImagenData() {
      ...
      try {
        ...
        if (this.imageFile !== this.imageCheck) {
          await api.patch(`/staff/deleteFileServices/services/${this.dataInfo.id}`, {
            filePath: this.imageCheck,
          });
          await api.patch(
            `/staff/addFileServices/services/${this.dataInfo.id}`,
            {
              file: this.imageFile,
            },
            {
        ...
        const res = await api.patch(
          `/staff/updateServicesData/${this.dataInfo.id}`,
          formUpdate
        );
